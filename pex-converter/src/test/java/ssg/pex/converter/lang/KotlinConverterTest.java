...
class KotlinConverterTest {
    ...
    private KotlinConverter converter;

    @BeforeEach
    void setUp() {
        converter = new KotlinConverter(ConversionConfig.defaults());
    }

    @Test
    void targetLanguageIsKotlin() {
        assertThat(converter.targetLanguage()).isEqualTo(TargetLanguage.KOTLIN);
    }
    ...
    @Test
    void convertsFunctionDefWithFun() {
        ...
        assertThat(result.isSuccess()).isTrue();
        assertThat(result.value()).contains("fun myFunc(a: Any, b: Any): Any");
    }
